...
def run_tests():
    ...
    with httpx.Client(app=app, base_url="http://test") as client:
        ...
        print(f"✓ List papers passed ({len(papers_data)} papers found)")
        ...
        print(f"✓ Analysis triggered successfully (Analysis ID: {analysis_id})")
        ...
        print(f"✓ ResNet report verified with {len(report['extracted_results'])} extracted metric comparisons")
        ...
        if qual_paper:
            ...
            print("✓ STRICT DATA INTEGRITY PASSED: Qualitative paper correctly generated NO fake charts")
        ...
        print("✓ User dashboard stats verified")
        ...
        print("✓ Role-Based Access Control (RBAC) verified: Forbidden for non-admin")

        # Admin User CRUD
        res = client.get("/api/users", headers=admin_headers)
        assert res.status_code == 200
        users_list = res.json()["data"]["items"]
        print(f"✓ Admin list users passed ({len(users_list)} users)")
